// S3 SKIFF BUOY — round the buoys and take the same dock end.
// Crossing that end off the slot fails the leg.
#pragma once
#include "art.h"
#include "console/system.h"

namespace skiff {

class Game : public gs::Cart {
public:
    const char* title() const override { return "S3 SKIFF BUOY"; }
    void init(gs::System& sys) override;
    void frame(gs::System& sys) override;

    void setBot(bool on) { bot_ = on; }
    bool over() const { return over_; }
    bool won() const { return won_; }
    int leg() const { return leg_; }
    float x() const { return x_; }
    float y() const { return y_; }
    float heading() const { return heading_; }
    float speed() const { return speed_; }
    float roundProg() const;
    const char* report() const { return report_; }
    // 0 title, 1 outbound, 2 rounding, 3 the end, 4 finished
    int marker() const;

private:
    enum class Mode { Title, Run, Pause, Fail, Win };

    struct Mark {
        float accum = 0, prev = 0;
        bool have = false, near = false;
    };
    struct Wake {
        float x = 0, y = 0, life = 0;
    };

    void begin();
    void showTitle();
    void startRun();
    void controls(float& steer, float& throttle);
    void pilot(float& steer, float& throttle);
    void physics(float dt, float steer, float throttle);
    void roundBuoy();
    void resolveEnd();
    void win();
    void fail();
    void chime(int notes);
    void blip(float freq);
    void audio(float dt);
    void camera();
    void draw();
    void hud(int col, int row, const char* s, int pal);
    void hudC(int row, const char* s, int pal);
    void spr(const gs::Mipped& m, float cx, float cy, float h, int pal, bool shadow = false);
    void place(const gs::Mipped& m, float wx, float wy, float worldH, int pal, float minPx = 0);
    void target(float& tx, float& ty) const;
    int hullFrame() const;

    gs::System* sys_ = nullptr;
    Art art_{};
    Mode mode_ = Mode::Title;
    bool bot_ = false;
    bool over_ = false;
    bool won_ = false;
    int leg_ = 0;
    int chimeN_ = 0, chimeStep_ = 0;
    int wakeCursor_ = 0;
    float t_ = 0, raceTime_ = 0, legTime_ = 0;
    float x_ = 0, y_ = 0, heading_ = 0, speed_ = 0, throttle_ = 0;
    float camX_ = 0, camY_ = 0, zoom_ = 0.36f;
    float tone0_ = 0, tone1_ = 0, chimeT_ = 0, thumpT_ = 0, wakeT_ = 0;
    float stuckT_ = 0, stuckX_ = 0, stuckY_ = 0;
    Mark mark_[3]{};
    Wake wakes_[24]{};
    char report_[160] = {};
};

}  // namespace skiff
