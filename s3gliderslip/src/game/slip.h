// S3 GLIDER SLIP — berth the flying boat before the tide turns.
// The clock is the other crew. When it runs out, the slip is theirs.
#pragma once
#include "art.h"
...
public:
    const char* title() const override { return "S3 GLIDER SLIP"; }
    ...
    bool afloat() const { return afloat_; }
    bool onSlip() const;
    // 0 title, 1 the approach, 2 over the slip, 3 on the water, 4 finished
    int marker() const;
...
private:
    ...
    void sprBox(const gs::Mipped& m, float cx, float top, float w, float h, int pal, int clip = gs::SCREEN_H);
    void project(float wx, float wy, float ax, float& sx, float& sy) const;
    void worldBox(float x0, float y0, float x1, float y1, const gs::Mipped& m, int pal, float ax, int clip);
    int wingFrame(float att) const;
    float tide() const;
    float waterLine() const;
    void mouth(float& a, float& b) const;
    void rivalAt(float& rx, float& rh) const;
    bool fuselageHits(float a, float b, float top) const;
    ...
    int puffN_ = 0;
    int lastSec_ = -1;
    float t_ = 0, race_ = 0, crew_ = 36.f;
    float x_ = 0, h_ = 0, v_ = 0, vy_ = 0, att_ = 0;
    float nose_ = 0, spoil_ = 0, hold_ = 0;
    float camX_ = 160, camH_ = 3, camS_ = 4.6f, anchorY_ = 150;
